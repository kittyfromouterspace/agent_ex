defmodule AgentEx.LLM.ProviderTest do
  use ExUnit.Case, async: true

  alias AgentEx.LLM.{Credentials, Model}

  describe "AgentEx.LLM.Provider.Anthropic" do
    test "id/0" do
      assert AgentEx.LLM.Provider.Anthropic.id() == :anthropic
    end

    test "label/0" do
      assert AgentEx.LLM.Provider.Anthropic.label() == "Anthropic"
    end

    test "transport/0" do
      assert AgentEx.LLM.Provider.Anthropic.transport() == AgentEx.LLM.Transport.AnthropicMessages
    end

    test "env_vars/0" do
      assert AgentEx.LLM.Provider.Anthropic.env_vars() == ["ANTHROPIC_API_KEY"]
    end

    test "default_models/0 returns models with pricing" do
      models = AgentEx.LLM.Provider.Anthropic.default_models()
    ...
    test "supports/0" do
      supports = AgentEx.LLM.Provider.Anthropic.supports()
    ...
    test "fetch_catalog/1 returns :not_supported" do
      assert AgentEx.LLM.Provider.Anthropic.fetch_catalog(%Credentials{}) == :not_supported
    end
  end

  describe "AgentEx.LLM.Provider.OpenAI" do
    test "id/0" do
      assert AgentEx.LLM.Provider.OpenAI.id() == :openai
    end

    test "default_models/0 includes embedding model" do
      models = AgentEx.LLM.Provider.OpenAI.default_models()
    ...
    test "supports/0 includes embeddings" do
      supports = AgentEx.LLM.Provider.OpenAI.supports()
      assert MapSet.member?(supports, :embeddings)
    end
  end

  describe "AgentEx.LLM.Provider.OpenRouter" do
    test "id/0" do
      assert AgentEx.LLM.Provider.OpenRouter.id() == :openrouter
    end

    test "transport/0 uses OpenAI compat" do
      assert AgentEx.LLM.Provider.OpenRouter.transport() ==
               AgentEx.LLM.Transport.OpenAIChatCompletions
    end

    test "request_headers/1 includes analytics headers" do
      headers = AgentEx.LLM.Provider.OpenRouter.request_headers(%Credentials{})
    ...
    test "fetch_catalog/1 without api key returns :not_supported" do
      assert AgentEx.LLM.Provider.OpenRouter.fetch_catalog(%Credentials{}) == :not_supported
    end
  end

  describe "AgentEx.LLM.Provider.Groq" do
    test "id/0" do
      assert AgentEx.LLM.Provider.Groq.id() == :groq
    end

    test "label/0" do
      assert AgentEx.LLM.Provider.Groq.label() == "Groq"
    end

    test "default_base_url/0" do
      assert AgentEx.LLM.Provider.Groq.default_base_url() == "https://api.groq.com/openai/v1"
    end

    test "default_models/0 returns models" do
      models = AgentEx.LLM.Provider.Groq.default_models()
    ...
    test "classify_http_error/3 — model_is_deactivated" do
      ...
      result = AgentEx.LLM.Provider.Groq.classify_http_error(400, body, [])
      assert {:model_not_found, nil} = result
    end

    test "classify_http_error/3 — default for unknown" do
      assert AgentEx.LLM.Provider.Groq.classify_http_error(429, "rate limited", []) == :default
    end
  end

  describe "AgentEx.LLM.Credentials" do
    test "resolve/1 returns :not_configured for unconfigured provider" do
      ...
      try do
        assert :not_configured = Credentials.resolve(AgentEx.LLM.Provider.Groq)
      after
    ...
    test "resolve/1 returns credentials when env var is set" do
      ...
      try do
        assert {:ok, %Credentials{api_key: "test-key-123"}} =
                 Credentials.resolve(AgentEx.LLM.Provider.Groq)
      after
        if original do
    ...
    test "available?/1 returns false for unconfigured provider" do
      ...
      try do
        refute Credentials.available?(AgentEx.LLM.Provider.Groq)
      after
        if original, do: System.put_env("GROQ_API_KEY", original)
      end
    end
