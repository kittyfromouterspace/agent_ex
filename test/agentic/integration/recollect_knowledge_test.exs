defmodule Agentic.Integration.RecollectKnowledgeTest do
  @moduledoc """
  Integration tests for the Recollect-backed Knowledge backend.

  These tests require a running PostgreSQL with the `recollect_test` database
  ...
  Then run with:
      ...
      mix test --include integration test/agentic/integration/recollect_knowledge_test.exs
  """
  ...
  alias Agentic.Persistence.Knowledge.Recollect, as: Backend

  setup_all do
    Application.ensure_all_started(:recollect)
    {:ok, _} = Recollect.TestRepo.start_link()

    migrations =
      case Application.app_dir(:recollect, "priv/repo/migrations") do
        {:error, _} ->
          Path.join([File.cwd!(), "..", "recollect", "priv", "repo", "migrations"])

        path ->
          path
      end

    :ok = Ecto.Migrator.run(Recollect.TestRepo, migrations, :up, all: true)
  end

  setup do
    pid = Ecto.Adapters.SQL.Sandbox.start_owner!(Recollect.TestRepo, shared: true)
