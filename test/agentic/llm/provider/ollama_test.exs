defmodule Agentic.LLM.Provider.OllamaTest do
  use ExUnit.Case, async: true

  alias Agentic.LLM.Model
  alias Agentic.LLM.Provider.Ollama
  ...
  describe "metadata" do
    ...
    test "transport is the Ollama transport" do
      assert Ollama.transport() == Agentic.LLM.Transport.Ollama
    end
    ...
    test "request_headers is empty" do
      assert Ollama.request_headers(%Agentic.LLM.Credentials{}) == []
    end
  end
  ...
  describe "fetch_catalog/1" do
    test "returns :not_supported when localhost is unreachable" do
      ...
      try do
        assert Ollama.fetch_catalog(%Agentic.LLM.Credentials{}) == :not_supported
      after
        if original,
