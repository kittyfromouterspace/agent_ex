defmodule Agentic.Loop.Stages.ToolPermissionTest do
  use ExUnit.Case, async: true

  alias Agentic.Loop.Stages.ToolExecutor
  ...
  describe "tool permission gating" do
    ...
    test "allows tool with modified input when on_tool_approval returns {:approved_with_changes, new_input}" do
      ...
      approval_callback = fn _tool_name, _input, _ctx ->
    ...
    test "passes through when no pending tool calls" do
      ...
      assert {:ok, result_ctx} =
               ToolExecutor.call(ctx, fn inner_ctx ->
                 {:ok, inner_ctx}
               end)
