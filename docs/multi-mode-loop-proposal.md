...
All new callbacks follow the existing pattern established in `Agentic.AgenticCallbacks` (see [Porting Analysis §15.1.5](./porting-analysis.md#1515-callback-architecture--homunculus-agentexcallbacks)). They are optional — `nil` values are handled gracefully.
