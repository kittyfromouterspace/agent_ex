# AgentEx Multi-Mode Loop — Implementation Proposal

> **Companion documents:**
>
> - [Gap Analysis](./gap-analysis.md) — Raschka's 6 components, proposed enhancements E1–E6
> - [Cross-Codebase Porting Analysis](./porting-analysis.md) — reusable code from homunculus and strategic_change_engine
> - [Persistence Strategy](./persistence-strategy.md) — behaviour definitions, Mneme integration, host app guide
> - [Implementation Plan](./implementation-plan.md) — phased order, testing strategy, file manifest, dependency cleanup

## 1. Goals

Add multiple execution modes to the agent loop so a single `AgentEx.run/1` call can behave very differently depending on the chosen mode:
...
3. **Phase transitions are validated by `AgentEx.Loop.Phase`.** A lightweight state machine module with per-mode transition maps. `Phase.transition(ctx, next_phase)` returns `{:ok, ctx}` or `{:error, {:invalid_transition, from, to}}`. Stages call this instead of mutating `ctx.phase` directly.
...
### 3.1 New fields on `AgentEx.Loop.Context`

The current `defstruct` (see `lib/agent_ex/loop/context.ex`) already has `phase: :execute`. New fields are added with defaults that match current behavior:
...
### 3.2 `AgentEx.Loop.Phase` — phase state machine
...
**File:** `lib/agent_ex/loop/phase.ex`

```elixir
defmodule AgentEx.Loop.Phase do
...
All new callbacks follow the existing pattern established in `AgentEx.AgentExCallbacks` (see [Porting Analysis §15.1.5](./porting-analysis.md#1515-callback-architecture--homunculus-agentexcallbacks)). They are optional — `nil` values are handled gracefully.
...
All stages implement `@behaviour AgentEx.Loop.Stage` (requiring `call/2` that receives `ctx` and `next`). `StopReasonRouter` is removed; `ModeRouter` handles all routing.

### 5.1 `ModeRouter`

**File:** `lib/agent_ex/loop/stages/mode_router.ex`
...
**File:** `lib/agent_ex/loop/stages/plan_builder.ex`
...
**File:** `lib/agent_ex/loop/stages/plan_tracker.ex`
...
**Plan completion detection** uses the `ContinuationDetector` patterns ported from homunculus (see [Porting Analysis §15.1.4](./porting-analysis.md#1514-plan-detection--homunculus-continuationdetector)). Primary strategy: LLM self-reports step completion in its response (prompt engineering). Fallback: after N turns per step, prompt the LLM to confirm completion.

### 5.4 `HumanCheckpoint`

**File:** `lib/agent_ex/loop/stages/human_checkpoint.ex`
...
**File:** `lib/agent_ex/loop/stages/verify_phase.ex`
...
**File:** `lib/agent_ex/loop/stages/workspace_snapshot.ex`
...
Uses the budget-aware assembly patterns from `Homunculus.Workspace.ContextAssembler` (see [Porting Analysis §15.1.2](./porting-analysis.md#1512-workspace-snapshot--homunculus-contextassembler--identity)).

---

## 6. Profile Changes

`AgentEx.Loop.Profile` (see `lib/agent_ex/loop/profile.ex`) gains new profile definitions. The current `stages/1` function pattern-matches on atoms and returns stage lists. New clauses are added:
...
`AgentEx.run/1` gains an optional `:mode` keyword. The mapping:
...
## 7. `AgentEx.run/1` API Changes

Current (see `lib/agent_ex.ex`):
```elixir
AgentEx.run(
  prompt: "...",
  workspace: "/path",
...
6. **Entry point** — add `:mode` opt to `AgentEx.run/1`, wire new callbacks

For the complete file manifest, see [Implementation Plan](./implementation-plan.md#file-manifest).
