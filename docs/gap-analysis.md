# Gap Analysis — AgentEx Multi-Mode Loop
...
Source: [Components of a Coding Agent](https://magazine.sebastianraschka.com/p/components-of-a-coding-agent) — Sebastian Raschka identifies six building blocks of a coding harness. Below is a gap analysis comparing AgentEx's current state (including the multi-mode proposal) against each component.
...
**What AgentEx has today:** The workspace path is passed to `run/1` and used in a generic system prompt: `"You are a helpful AI assistant working in #{workspace}."` (see `lib/agent_ex.ex:84`). The `Workspace.Service` module scaffolds workspaces with identity files, but nothing is automatically gathered or injected at session start.
...
**What AgentEx has today:** The full `messages` list is rebuilt and sent to `llm_chat` every turn as a flat list. There is no concept of a stable prefix vs volatile suffix. No cache boundary markers are passed to the LLM provider.
...
**What AgentEx has today:** This is the strongest area. Structured tool schemas (string-keyed), `Tools.execute/3` dispatch, CircuitBreaker (`lib/agent_ex/circuit_breaker.ex`), path traversal protection in `resolve_path/2`, and output truncation for bash (`@max_output_bytes`). The `Gateway` module handles lazy tool discovery and activation with an LRU budget.
...
**What AgentEx has today:** `ContextGuard` handles compaction — when context exceeds 80%, it summarizes older messages into a deterministic handoff message. But the summarization is crude (rule-based, first 200 chars of each message, tool names listed). No LLM-based summarization.
...
**What AgentEx has today:** `ContextKeeper` provides in-process working memory (facts + key-value working set) with TTL support and fact supersession. On termination, it flushes to `MEMORY.md`. The `Memory` tools provide `memory_query`/`memory_write`/`memory_note`/`memory_recall` backed by callbacks to an external knowledge store. But there is no full transcript storage and no session resumption.

**Gap:**
- **No full transcript** — once a session ends, the complete history is gone
- **No session resumption** — `AgentEx.run/1` is fire-and-forget; no way to continue a previous session
...
**Proposed enhancement (E4):** See [Persistence Strategy](./persistence-strategy.md) for the `AgentEx.Persistence.Transcript` behaviour and session resumption design.
...
**What AgentEx has today:** Zero support. No subagent spawning, no delegation, no parallel execution. Every task runs through the single main pipeline.
