defmodule Agentic.ModelRouter.Analyzer do
  @moduledoc """
  ...
  defp analyze_via_llm(request, context_summary, llm_chat, session_id) do
    ...
    case llm_chat.(params) do
      ...
      {:error, reason} ->
        ...
        Agentic.Telemetry.event([:model_router, :analysis, :fallback], %{}, %{
          session_id: session_id,
          from: :llm,
          to: :heuristic,
  ...
  defp parse_analysis(text) do
    case extract_json(text) do
      ...
      _ ->
        ...
        Agentic.Telemetry.event([:model_router, :analysis, :parse_failure], %{}, %{})
