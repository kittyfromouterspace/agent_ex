defmodule Agentic.Persistence.Knowledge.Recollect do
  ...
  Delegates to Recollect's Tier 2 API (entries + edges) for hybrid vector + graph
  knowledge search with auto-embedding.

  ## Configuration

  Recollect must be configured with a repo and embedding provider:
  ...
  def search(query, opts) do
    ...
    case Recollect.search(query, search_opts) do
      {:ok, context_pack} ->
        entries =
          (context_pack.entries ++ Map.get(context_pack, :related_entries, []))
          |> Enum.map(&recollect_entry_to_map/1)
  ...
  def create_entry(entry, opts) do
    ...
    case Recollect.remember(content, recollect_opts) do
      {:ok, recollect_entry} ->
        {:ok, recollect_entry_to_map(recollect_entry)}
  ...
  def get_entry(entry_id, _opts) do
    ...
    case repo.get(Recollect.Schema.Entry, entry_id) do
      nil -> {:error, :not_found}
      entry -> {:ok, recollect_entry_to_map(entry)}
    end
  end
  ...
  def get_edges(entry_id, direction, _opts) do
    ...
    query =
      case direction do
        :from ->
          from(e in Recollect.Schema.Edge, where: e.source_entry_id == ^entry_id)

        :to ->
          from(e in Recollect.Schema.Edge, where: e.target_entry_id == ^entry_id)
      end

    edges = repo.all(query)
    {:ok, Enum.map(edges, &recollect_edge_to_map/1)}
  end
  ...
  def create_edge(from_id, to_id, relation, opts) do
    ...
    case Recollect.connect(from_id, to_id, relation, edge_opts) do
      {:ok, edge} ->
        {:ok, recollect_edge_to_map(edge)}
  ...
  def recent(scope_id, opts) do
    limit = opts[:limit] || 20
    entries = Recollect.Knowledge.recent(scope_id, limit: limit)
    {:ok, Enum.map(entries, &recollect_entry_to_map/1)}
  end

  @impl true
  def supersede(scope_id, entity, relation, _new_value) do
    _count = Recollect.Knowledge.supersede(scope_id, entity, relation, "")
    {:ok, []}
  end

  defp recollect_entry_to_map(%Recollect.Schema.Entry{} = entry) do
    %{
      id: entry.id,
  ...
  defp recollect_entry_to_map(%{} = entry) do
    %{
  ...
  defp recollect_entry_to_map(_), do: nil

  defp recollect_edge_to_map(%Recollect.Schema.Edge{} = edge) do
    %{
      id: edge.id,
