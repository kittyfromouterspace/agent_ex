defmodule Agentic.Memory.ContextKeeper do
  @moduledoc """
  ...
  def running?(workspace_id) do
    case Registry.lookup(Agentic.Memory.ContextKeeperRegistry, workspace_id) do
      [{_pid, _}] -> true
      [] -> false
    end
  end
  ...
  defp via_tuple(workspace_id) do
    {:via, Registry, {Agentic.Memory.ContextKeeperRegistry, workspace_id}}
  end
  ...
  defp flush_to_memory_md(state) do
    ...
    if lines != [] do
      ...
      storage = Agentic.Storage.Context.for_workspace(state.workspace_path)

      case Agentic.Storage.Context.read(storage, "MEMORY.md") do
        {:ok, existing} ->
          updated = prune_and_append(existing, new_section)
          Agentic.Storage.Context.write(storage, "MEMORY.md", updated)

        {:error, _} ->
          Agentic.Storage.Context.write(storage, "MEMORY.md", "# Memory\n" <> new_section)
      end
