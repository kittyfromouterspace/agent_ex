defmodule Agentic.LLM do
  ...
  def chat(params, opts \\ []) do
    provider =
      case Keyword.get(opts, :provider) do
        nil -> raise ArgumentError, "Agentic.LLM.chat/2 requires :provider option"
  ...
  defp walk_routes([], _params, tier, _llm_chat, _opts, last_error) do
    Logger.debug("Agentic.LLM.chat_tier: all routes for tier #{tier} exhausted")
  ...
  defp walk_routes([route | rest], params, tier, llm_chat, opts, _last) do
    Logger.debug(
      "Agentic.LLM.chat_tier: trying #{route.provider_name}/#{route.model_id} (tier #{tier})"
    )
    ...
    case result do
      ...
      {:error, _reason} = err ->
        ...
        Logger.warning(
          "Agentic.LLM.chat_tier: #{route.provider_name}/#{route.model_id} failed (#{failure}); trying next"
        )
  ...
  defp classify_tier_error({:error, %{message: msg}}) when is_binary(msg),
    do: classify_tier_error({:error, msg})
  ...
  defp classify_tier_error({:error, reason}) when is_binary(reason) do
    cond do
      ...
      String.contains?(reason, "401") or String.contains?(reason, "403") ->
        :auth_error
  ...
  defp resolve_embedding_target(tier, opts) do
    ...
    if explicit_model && explicit_provider do
      ...
      preferred_id = explicit_model || Agentic.Config.embedding_model()
