defmodule Agentic do
  ...
  alias Agentic.Loop.Engine
  alias Agentic.Loop.Profile
  alias Agentic.ModelRouter
  alias Agentic.Tools
  alias Agentic.Tools.Activation
  ...
  defp resolve_strategy(opts) do
    case Keyword.get(opts, :strategy) do
      ...
      id when is_atom(id) ->
        case Agentic.Strategy.Registry.fetch(id) do
          nil -> id
          mod -> mod
        end
    end
  ...
  defp run_single(opts) do
    ...
    initial_phase = Agentic.Loop.Phase.initial_phase(mode)
  ...
  def new_workspace(path, opts \\ []) do
    Agentic.Workspace.Service.create_workspace(path, opts)
  end
  ...
  def resume(opts) do
    ...
    workspace = Keyword.fetch!(opts, :workspace)
    callbacks = Keyword.fetch!(opts, :callbacks)
    backend = Keyword.get(opts, :transcript_backend, Agentic.Persistence.Transcript.Local)
