defmodule AgentEx.LLM.Gateway do
  @moduledoc """
  ...
  capture full traffic details, and emit `:agent_ex` telemetry events
  that flow into Worth's X-Ray panel.

  ## Usage (from web controller)

      {status, headers, body} = AgentEx.LLM.Gateway.proxy(
        :anthropic,
        "POST",
        "/v1/messages",
        req_headers,
  ...
  the proxy endpoint into spawned CLI processes:
      ...
      config
      |> put_in([:env, "ANTHROPIC_BASE_URL"], AgentEx.LLM.Gateway.base_url(:anthropic))

  """

  alias AgentEx.LLM.{Credentials, ProviderRegistry, Response}
  ...
  def base_url(provider_id) do
    case Application.get_env(:agent_ex, :llm_gateway_base_url) do
      nil -> nil
    ...
    case Credentials.resolve(provider) do
      {:ok, creds} ->
        ...
        case Req.post(request.url,
          ...
          {:error, exception} ->
            duration = System.monotonic_time() - start_time
            emit_gateway_stop(call_id, provider.id(), nil, duration, %{}, %{error: inspect(exception)})
            {502, [{"content-type", "application/json"}], Jason.encode!(%{error: %{message: Exception.message(exception), type: "gateway_error"}})}
        end

      :not_configured ->
        duration = System.monotonic_time() - start_time
        emit_gateway_stop(call_id, provider.id(), 401, duration, %{}, %{})
        {401, [{"content-type", "application/json"}], Jason.encode!(%{error: %{message: "Provider not configured", type: "auth_error"}})}
    end
  end
        ...
        case Req.request(method: :post,
          ...
          {:ok, %{status: 200, body: %Req.Response.Async{} = async_body, headers: resp_headers}} ->
            ...
            on_done = fn ->
              ...
              emit_gateway_stream_stop(call_id, provider.id(), 200, duration, %{}, get(chunk_count), ttft_ms, %{})
            end

            wrapped = %AgentEx.LLM.Gateway.AsyncStream{
              async: async_body,
              on_chunk: fn ->
                bump(chunk_count)
            ...
            resp_headers = normalize_headers(resp_headers)
            {200, [{"content-type", "text/event-stream"}, {"cache-control", "no-cache"} | resp_headers], wrapped_stream}

          {:ok, %{status: status, body: resp_body, headers: resp_headers}} when is_binary(resp_body) or is_map(resp_body) ->
            duration = System.monotonic_time() - start_time
            emit_gateway_stream_stop(call_id, provider.id(), status, duration, %{}, 0, nil, resp_body)
            resp_headers = normalize_headers(resp_headers)
            {status, resp_headers, Jason.encode!(resp_body)}

          {:error, exception} ->
            duration = System.monotonic_time() - start_time
            emit_gateway_stream_stop(call_id, provider.id(), 502, duration, %{}, 0, nil, %{error: inspect(exception)})
            {502, [{"content-type", "application/json"}], Jason.encode!(%{error: %{message: Exception.message(exception), type: "gateway_error"}})}
        end
  ...
  defp emit_gateway_stream_stop(call_id, provider_id, status, duration, usage, chunk_count, ttft_ms, final_acc) do
    AgentEx.Telemetry.event(
      [:gateway, :request, :stop],
      %{
        duration: duration,
  ...
  defp ref(initial), do: :atomics.new(1, signed: false) |> tap(&:atomics.put(&1, 1, encode_ref(initial)))
