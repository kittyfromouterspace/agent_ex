defmodule AgentEx.Protocol.ACP.Session do
  @moduledoc """
  ...
  ## Usage
      ...
      {:ok, session} = AgentEx.Protocol.ACP.Session.connect(
        client: acp_client,
        workspace: "/path/to/project",
        permission_policy: :ask
      )

      {:ok, response, session} = AgentEx.Protocol.ACP.Session.prompt(
        session,
        [%{"type" => "text", "text" => "Hello"}]
      )

      :ok = AgentEx.Protocol.ACP.Session.cancel(session)
      :ok = AgentEx.Protocol.ACP.Session.close(session)
  """

  require Logger

  alias AgentEx.Protocol.ACP.Types
  ...
  def cancel(%__MODULE__{} = session, _opts) do
    ...
    AgentEx.Protocol.ACP.Client.notify(session.client, "session/cancel", params)
  end
  ...
  def close(%__MODULE__{} = session, _opts) do
    AgentEx.Protocol.ACP.Client.stop(session.client)
  end
  ...
  def load(%__MODULE__{} = session, session_id) do
    if can_load_session?(session) do
      ...
      case AgentEx.Protocol.ACP.Client.request(session.client, "session/load", params,
             timeout: 30_000
           ) do
        {:ok, _result} ->
  ...
  def set_mode(%__MODULE__{} = session, mode_id) do
    ...
    case AgentEx.Protocol.ACP.Client.request(session.client, "session/set_mode", params) do
      {:ok, _} -> :ok
  ...
  defp setup_notification_handler(session) do
    ...
    AgentEx.Protocol.ACP.Client.request(session.client, "_set_notification_handler", %{
  ...
  defp initialize(session, client_info, client_capabilities) do
    ...
    case AgentEx.Protocol.ACP.Client.request(session.client, "initialize", params,
           timeout: 10_000
         ) do
  ...
  defp create_session(session) do
    ...
    case AgentEx.Protocol.ACP.Client.request(session.client, "session/new", params,
           timeout: 15_000
         ) do
