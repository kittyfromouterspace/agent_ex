defmodule AgentEx.Protocol.ACP do
  @moduledoc """
  Generic ACP (Agent Client Protocol) implementation.

  Implements `AgentEx.AgentProtocol` for any ACP-compatible agent.
  ...
  ## Usage
      ...
      # Direct use
      {:ok, session_id} = AgentEx.Protocol.ACP.start(
  ...
  alias AgentEx.Protocol.ACP.Session
  ...
  def start(backend_config, _ctx) do
    ...
    case AgentEx.Protocol.ACP.Client.start_link(client_opts) do
      {:ok, client} ->
        connect_opts = [
          client: client,
          workspace: workspace,
    ...
    fn id, method, params ->
      case method do
        "session/request_permission" ->
          AgentEx.Protocol.ACP.Permission.handle_request(
            %{"id" => id, "params" => params},
